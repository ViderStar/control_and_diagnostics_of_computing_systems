"""Runner for lab 5 (pattern-sensitive faults)."""

from __future__ import annotations

import logging

from configs.cfg import LAB5_FAULT_SAMPLES, LAB5_RAM_BITS, LAB5_SIM_BITS
from helpers.memory_faults import build_pattern_faults
from helpers.memory_runner import CoverageReport, run_suite
from lab5.tests import get_lab5_tests

logger = logging.getLogger(__name__)


def run_lab5(sim_size: int = LAB5_SIM_BITS) -> CoverageReport:
    tests = get_lab5_tests()
    scenarios = build_pattern_faults(sim_size, passive_neighbors=9, active_neighbors=3, sample=LAB5_FAULT_SAMPLES)
...
def _log_summary(report: CoverageReport, sim_size: int) -> None:
    ...
    logger.info("=== Lab 5: Pattern-sensitive faults ===")
    ...
    for test in report.tests:
        ...
        estimated_ops = test.estimate(LAB5_RAM_BITS)
        logger.info(
            f"  Complexity {test.complexity_label}, ~{estimated_ops:,} operations for {LAB5_RAM_BITS:,} bits",
        )
